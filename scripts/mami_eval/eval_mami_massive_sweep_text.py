...
def main():
    ...
    # Changed output name to prevent mixing with non-text runs
    out_csv = "/scratch/results/mami_massive_sweep_with_text_results.csv"
    os.makedirs(os.path.dirname(out_csv), exist_ok=True)

    # ------------------------------------------
    # MERGE DATASETS (Labels + Text)
    # ------------------------------------------
    ...
    print(f"Total merged samples ready: {len(mami_df)}")
        ...
        for pattern_name, pattern_func in patterns.items():
            for alpha in alphas_to_test:
                ...
                for index, row in mami_df.iterrows():
                    ...
                    # Safely extract text transcription
                    ...
                    try:
                        ...
                        # 2. DYNAMIC PROMPT (Injected Text)
                        dynamic_instruction = f"Meme Text: '{text_transcription}'\n\nTask: Identify if this meme contains misogyny.\nAllowed Labels: 'misogynous' or 'clean'.\nAnswer:"
