...
# ==========================================
# Baseline evaluation: same held-out random geometry test set as the fine-tuned script
# No LoRA adapter is attached in this file.
# ==========================================
# Configuration
# ==========================================
TOP_20_EMOJIS = {
...
# ==========================================
def main():
    parser = argparse.ArgumentParser(description="Evaluate the baseline MemeLens-VLM on the same unseen random emoji geometries at fixed alpha=0.8.")
    ...
    default_name = (
        "mami_baseline_unseen_randomgeom_with_text_alpha08.csv"
        if include_text else
        "mami_baseline_unseen_randomgeom_no_text_alpha08.csv"
    )
    ...
    temp_csv = os.path.join(RESULTS_DIR, f"temp_baseline_unseen_randomgeom_{args.mode}_rank_{local_rank}.csv")
    ...
    if local_rank == 0:
        print("Using baseline model only: no LoRA adapter is attached.")
    model = base_model
    model.eval()
    ...
    if local_rank == 0:
        ...
        for rank in range(world_size):
            rank_csv = os.path.join(RESULTS_DIR, f"temp_baseline_unseen_randomgeom_{args.mode}_rank_{rank}.csv")
