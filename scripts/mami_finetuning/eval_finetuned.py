...
def main():
    ...
    # Each GPU gets its own temporary CSV to prevent file locking crashes
    temp_csv = f"/scratch/results/temp_rank_{local_rank}_results.csv"
    os.makedirs(os.path.dirname(OUT_CSV), exist_ok=True)
    with open(temp_csv, "w") as f:
        f.write("file_name,true_label,emoji_name,pattern,alpha,pred_label\n")

    # ------------------------------------------
    # DATA PREPARATION (Only Rank 0 prints to keep terminal clean)
    # ------------------------------------------
    ...
    # ------------------------------------------
    # THE SHARDING LOGIC: Build the global list of tasks, then split it
    # ------------------------------------------
    all_tasks = []
        ...
        for emoji_name, emoji_path in available_emojis.items():
            for pattern_name, pattern_func in patterns.items():
                for alpha in alphas_to_test:
                    # Skip redundant clean (alpha 0.0) runs
                    ...
                    all_tasks.append({
                        ...
                        "text": str(row['Text Transcription']) if pd.notna(row['Text Transcription']) else "",
    ...
    # INFERENCE LOOP
    # ------------------------------------------
    # Only show the progress bar on GPU 0 so the terminal stays clean
    pbar = tqdm.tqdm(total=len(my_tasks), disable=(local_rank != 0), desc="Inferencing")
    ...
    # CLEANUP & MERGE
    # ------------------------------------------
    # Wait for all 6 GPUs to finish their slices
    dist.barrier()
    
    if local_rank == 0:
        print("All GPUs finished. Merging temporary files into final CSV...")
        all_dfs = []
        for i in range(world_size):
            rank_csv = f"/scratch/results/temp_rank_{i}_results.csv"
            all_dfs.append(pd.read_csv(rank_csv))
            os.remove(rank_csv) # Clean up temp file
